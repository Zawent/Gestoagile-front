...
export const LoginUsuario = () => {
  ...
  // CSS vars derivadas del branding real
  const cssVars = {
    '--primary':    primary,
    '--accent-bg':  primary,
    '--sidebar':    quaternary,
  ...
  // ── Skeleton de carga ── mostrado mientras llega el branding
  if (brandingLoading) {
    return (
      <div style={{
        minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center',
        background: '#f8f8fc', fontFamily: 'sans-serif',
      }}>
        <style>{`
          @keyframes pulse { 0%,100%{opacity:0.4} 50%{opacity:1} }
          .sk { border-radius:12px; background:#e2e8f0; animation:pulse 1.4s ease-in-out infinite; }
        `}</style>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .login-root {
          ...
          font-family:'DM Sans',sans-serif; color:#1a1a2e; position:relative; overflow:hidden;
          background: color-mix(in srgb, var(--accent-bg) 8%, #f8f8fc);
        }
        .login-root__blob { position:fixed; border-radius:50%; filter:blur(100px); opacity:0.28; pointer-events:none; z-index:0; }
        ...
        .login-box {
          ...
          border:1px solid rgba(255,255,255,0.6); border-radius:24px; padding:2.5rem 2rem;
        ...
        .success-overlay {
          ...
          background:rgba(255,255,255,0.96); backdrop-filter:blur(8px); z-index:10;
        ...
        .success-title { font-family:'Sora',sans-serif; font-size:1.3rem; font-weight:800; color:#1a1a2e; letter-spacing:-0.02em; }
        .success-sub   { font-size:0.88rem; color:#7a7a9a; }
        ...
        .login-logo { width:80px; height:80px; object-fit:contain; border-radius:20px; box-shadow:0 8px 30px color-mix(in srgb, var(--primary) 20%, transparent); margin-bottom:0.85rem; }
        .login-company { font-family:'Sora',sans-serif; font-size:1.35rem; font-weight:700; color:#1a1a2e; letter-spacing:-0.02em; }
        .login-heading { font-family:'Sora',sans-serif; font-size:1.75rem; font-weight:800; color:#1a1a2e; margin-bottom:0.35rem; }
        .login-subheading { font-size:0.9rem; color:#7a7a9a; margin-bottom:2rem; }

        .login-form { display:flex; flex-direction:column; gap:1.25rem; }
        .field-label { display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem; }
        .field-label span { font-size:0.85rem; font-weight:600; color:#3a3a5c; }
        .field-label a { font-size:0.82rem; font-weight:600; color:var(--primary); text-decoration:none; transition:opacity .2s; }
        .field-label a:hover { opacity:.75; }
        .field-input-wrap { position:relative; }
        .field-input {
          width:100%; padding:0.9rem 1rem; border:1.5px solid #e2e2ee; border-radius:14px;
          background:#fafafd; font-family:'DM Sans',sans-serif; font-size:0.95rem; color:#1a1a2e;
        ...
        .field-input--error { border-color:#f87171 !important; box-shadow:0 0 0 4px rgba(248,113,113,0.12) !important; }
        .field-input--padded-right { padding-right:3rem; }

        .visibility-btn { position:absolute; right:0.9rem; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; color:#a0a0be; display:flex; align-items:center; transition:color .2s; padding:0; }
        ...
        .remember-label { font-size:0.85rem; color:#5a5a78; cursor:pointer; user-select:none; }

        .error-msg { display:flex; align-items:center; gap:0.5rem; padding:0.75rem 1rem; background:rgba(248,113,113,0.1); border:1px solid rgba(248,113,113,0.3); border-radius:12px; font-size:0.85rem; color:#dc2626; animation:shakeX 0.4s ease; }
        ...
        .btn-submit {
          ...
          font-family:'Sora',sans-serif; font-size:1rem; font-weight:700; border:none; border-radius:14px; cursor:pointer;
          box-shadow:0 8px 24px color-mix(in srgb, var(--primary) 30%, transparent);
          transition:opacity .2s, transform .1s; letter-spacing:-0.01em;
          display:flex; align-items:center; justify-content:center; gap:0.5rem; min-height:52px;
        }
        ...
        .btn-back { width:100%; padding:0.9rem; background:transparent; color:#7a7a9a; font-family:'DM Sans',sans-serif; font-size:0.9rem; font-weight:500; border:1.5px solid #e2e2ee; border-radius:14px; cursor:pointer; transition:background .2s, color .2s, border-color .2s; display:flex; align-items:center; justify-content:center; gap:0.4rem; }
        .btn-back:hover { background:#f2f2fa; color:#3a3a5c; border-color:#c8c8e0; }
        .btn-back .material-symbols-outlined { font-size:18px; }

        .login-footer { margin-top:1.75rem; text-align:center; font-size:0.85rem; color:#9090b0; }
