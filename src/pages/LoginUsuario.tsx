...
export const LoginUsuario = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!empresaId) throw new Error('No se encontró el contexto de empresa.')

      // publicFetch porque el login no lleva Authorization ni slug en la URL
      // la ruta es: /api/{slug}/login  → la armamos manualmente
      const BASE = import.meta.env.VITE_API_URL ?? '/api'
      const res  = await fetch(`${BASE}/${slug}/login`, {
        method:  'POST',
      ...
      localStorage.setItem('user_rol_nombre', data.user.rol_nombre)
      localStorage.setItem('permisos', JSON.stringify(data.permisos))
      // empresa_id, empresa_slug y empresa_nombre ya vienen del paso 1
  ...
  const cssVars = { '--primary': primary, '--accent-bg': primary, '--sidebar': quaternary } as React.CSSProperties
    ...
    <div style={cssVars} className="login-root">
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .login-box{width:100%;max-width:420px;background:rgba(255,255,255,0.72);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.6);border-radius:24px;padding:2.5rem 2rem;box-shadow:0 8px 32px rgba(0,0,0,.08);position:relative;animation:loginBoxIn .35s ease;}
        @keyframes loginBoxIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
        .success-overlay{position:absolute;inset:0;border-radius:24px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.25rem;background:rgba(255,255,255,.96);z-index:10;animation:fadeInOverlay .4s ease forwards;}
        @keyframes fadeInOverlay{from{opacity:0;transform:scale(.97)}to{opacity:1;transform:scale(1)}}
        .success-icon-wrap{width:72px;height:72px;border-radius:50%;background:color-mix(in srgb,var(--primary) 12%,transparent);display:flex;align-items:center;justify-content:center;}
        .success-icon-wrap .material-symbols-outlined{font-size:38px;color:var(--primary);}
        .success-title{font-family:'Sora',sans-serif;font-size:1.3rem;font-weight:800;color:#1a1a2e;}
        ...
        .login-logo{width:80px;height:80px;object-fit:contain;border-radius:20px;margin-bottom:.85rem;}
        .login-company{font-family:'Sora',sans-serif;font-size:1.35rem;font-weight:700;color:#1a1a2e;}
        ...
        .field-input{width:100%;padding:.9rem 1rem;border:1.5px solid #e2e2ee;border-radius:14px;background:#fafafd;font-family:'DM Sans',sans-serif;font-size:.95rem;color:#1a1a2e;outline:none;transition:border-color .2s,box-shadow .2s;}
        ...
        .visibility-btn{position:absolute;right:.9rem;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#a0a0be;display:flex;align-items:center;padding:0;}
        ...
        .error-msg{display:flex;align-items:center;gap:.5rem;padding:.75rem 1rem;background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.3);border-radius:12px;font-size:.85rem;color:#dc2626;}
        .error-msg .material-symbols-outlined{font-size:18px;flex-shrink:0;}
        .btn-submit{width:100%;padding:1rem;background:var(--primary);color:#fff;font-family:'Sora',sans-serif;font-size:1rem;font-weight:700;border:none;border-radius:14px;cursor:pointer;transition:opacity .2s;display:flex;align-items:center;justify-content:center;gap:.5rem;min-height:52px;}
        ...
        .btn-back{width:100%;padding:.9rem;background:transparent;color:#7a7a9a;font-family:'DM Sans',sans-serif;font-size:.9rem;font-weight:500;border:1.5px solid #e2e2ee;border-radius:14px;cursor:pointer;transition:background .2s;display:flex;align-items:center;justify-content:center;gap:.4rem;}
        .btn-back:hover{background:#f2f2fa;}
        ...
        <div className="login-box">
          ...
          <form onSubmit={handleLogin} className="login-form">
            ...
            <button type="submit" className="btn-submit" disabled={isLoading||isSuccess}>
              {isLoading ? <><div className="spinner"/> Verificando…</> : 'Iniciar sesión'}
            </button>
          ...
          <p className="login-footer">¿No tienes cuenta? <a href="#" style={{color:'var(--primary)',fontWeight:600}}>Crear una cuenta</a></p>
        </div>
      </div>
    </div>
  )
